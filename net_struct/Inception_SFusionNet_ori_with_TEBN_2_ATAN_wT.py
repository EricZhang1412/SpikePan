...
class FusionNet(nn.Module):
    def __init__(self, in_ch, mid_ch, out_ch, T1:int,T2:int):
        super(FusionNet, self).__init__()
        self.T1 = T1
        self.T2 = T2
        self.T = self.T1+self.T2
        self.conv1 = layer.Conv2d(2, mid_ch, kernel_size=3, stride=1, padding=1, bias=False)
        self.conv11 = layer.Conv2d(4, mid_ch, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        ...
        x1 = rearrange(x, 'n (T c1) h w -> T n c1 h w',T = 4)
        x2 = rearrange(x, 'n (T c1) h w -> T n c1 h w',T = 2)
        x1 = self.conv1(x1)
        x2 = self.conv11(x2)
