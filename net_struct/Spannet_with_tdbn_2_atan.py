...
class Resblock(nn.Module):
    def __init__(self,in_channels,out_channels,T:int):
        ...
        self.T = T
        # channel = 32
        self.conv1 = layer.Conv2d(in_channels=in_channels, out_channels=out_channels, kernel_size=3, stride=1, padding=1,
                                bias=True)
        self.bn1 = tdBatchNorm(alpha=1.0, v_th=1.0, num_features=out_channels)
        self.conv2 = layer.Conv2d(in_channels=out_channels, out_channels=out_channels, kernel_size=3, stride=1, padding=1,
                                bias=True)
...
class PanNet(nn.Module):
    def __init__(self, in_ch,mid_ch,out_ch,T: int):
        ...
        self.res1 = Resblock(mid_ch,mid_ch,T=self.T)
        self.res2 = Resblock(mid_ch,mid_ch,T=self.T)
        self.res3 = Resblock(mid_ch,mid_ch,T=self.T)
        self.res4 = Resblock(mid_ch,mid_ch,T=self.T)
        self.conv2 = layer.Conv2d(in_channels=channel, out_channels=out_ch, kernel_size=3, stride=1, padding=1,
                               bias=True)
        self.conv3 = layer.Conv2d(in_channels=channel, out_channels=out_ch, kernel_size=3, stride=1, padding=1,
                               bias=True)
