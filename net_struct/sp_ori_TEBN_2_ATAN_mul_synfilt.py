...
class FusionNet(nn.Module):
    def __init__(self, in_ch, mid_ch, out_ch, T:int):
        ...
        self.T = T
        self.in_ch = in_ch 
        self.conv1 = layer.Conv2d(4, mid_ch, kernel_size=3, stride=1, padding=1, bias=False)
    ...
    def forward(self, x):
        x = x.repeat(1, 4, 1, 1)
        # change the shape of x to [T, N, C1, H, W], where T x C1 = C
        x = rearrange(x, 'N (T C1) H W -> T N C1 H W', T=self.T)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.lif1(x)
        lif1_out = x
